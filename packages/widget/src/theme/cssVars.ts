...
function applyPariCardLayoutVars(
  ...
  style[`${prefix}-meta-direction`] = "row";
  style[`${prefix}-meta-align`] = "center";
  style[`${prefix}-meta-justify`] = shouldStack ? "flex-start" : "space-between";
  style[`${prefix}-meta-gap`] = shouldStack ? "4px" : "8px";
...
function applyLayoutVars(layout: ToncastWidgetLayout | undefined, style: StyleVars): void {
  ...
  const mobileColumns = normalizeColumns(grid.mobile, DEFAULT_GRID_LAYOUT.mobile);
  const tabletColumns = normalizeColumns(grid.tablet, DEFAULT_GRID_LAYOUT.tablet);
  const desktopColumns = normalizeColumns(grid.desktop, DEFAULT_GRID_LAYOUT.desktop);
